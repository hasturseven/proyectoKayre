...
def extraer_datos_evaluacion(texto):
    ...
    # Nueva parte: Clasificar tipo de dispensación
    texto_min = texto.lower()  # Normalizar texto a minúsculas

    # 1. Parcialmente adherente relacionado a EPS o IPS
    parcial_regex = re.search(
        r'parcia[lm]*ente\s+adh[eé]rente.*?(eps|epps|ep\s|ips|ipss|ip\s)',
        texto_min
    )

    # 2. Totalmente adherente, completamente adherente o simplemente adherente
    completo_regex = re.search(
        r'(totalmente|completamente)?\s*adh[eé]rente',
        texto_min
    )

    # 3. No adherente y sin dispensación de medicamentos
