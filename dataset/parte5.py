...
for paciente_id, paciente_info in data.items():
    ...
    # Unir textos
    todos_meds = f"{tratamiento_principal} {conciliacion_meds}"

    # Separadores comunes
    separadores = [",", "\n", ".", ";", "+"]
    for sep in separadores:
        todos_meds = todos_meds.replace(sep, "|")

    # Lista limpia de medicamentos
    lista_meds = [med.strip() for med in todos_meds.split("|") if med.strip()]
    ...
    for med in lista_meds:
        palabras = med.split()
        if len(palabras) >= 2:
            nombre_base = " ".join(palabras[:2])  # Ej: "vitamina d"
        elif palabras:
...
columnas_finales = otras_columnas + columnas_clasificacion_enfermedades + columnas_clasificacion_clinimetria+ ["cambio_en_medicacion"]
