...
def extraer_datos_evaluacion(texto):
    ...
    # Adherencia general
    match = re.search(
        r'paciente\s+(totalmente\s+adherente|no\s+adherente|parcialmente\s+adherente|adherente)\s+(al\s+)?tratamiento',
        texto, re.IGNORECASE
    )
    info_extraida["adherencia"] = match.group(1).strip() if match else "No especificado"

    import difflib #TODO BIBLIOTECA ERRORES TIPOGRAFICOS

    # Adherencia Morisky
    match = re.search(
        r'Adherencia\s*:\s*Conforme\s+a\s+Test\s+de\s+adherencia\s+Morisky\s+Green\s+el\s+paciente\s+es\s+([_\w\s]+)',
        texto, re.IGNORECASE
    )
    if match:
        valor = match.group(1).strip().upper()

        # Si es guion bajo o vacío, lo tratamos como "ADHERENTE"
        if valor == "_" or valor == "":
            info_extraida["adherencia_morisky"] = "ADHERENTE"
        else:
            opciones_validas = ["ADHERENTE", "PARCIALMENTE ADHERENTE", "NO ADHERENTE"]
            valor_corregido = difflib.get_close_matches(valor, opciones_validas, n=1, cutoff=0.8)

            if valor_corregido:
                info_extraida["adherencia_morisky"] = valor_corregido[0]
            else:
                info_extraida["adherencia_morisky"] = "NO EVALUABLE"
    else:
        info_extraida["adherencia_morisky"] = "NO EVALUABLE"
